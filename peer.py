...
class peerMain:
    ...
    def __init__(self):
        ...
        while choice != "3":
            ...
            # if peer is not logged in and exits the program
            elif choice == "3":
                self.logout(2)
            ...
            # to enter the username of the user that is wanted to be chatted
                ...
                if searchStatus != None and searchStatus != 0:
                    ...
                    #searchStatus[0] = ip, searchStatus[1] = port, name,
    ...
    def addUsers(self, roomid, peerUserName):
        ...
        #Join room chat 
    def joinRoomChat(self, roomid, peerUserName):
        message = "JOINROOMCHAT " + roomid + " " + peerUserName
        self.tcpClientSocket.send(message.encode())
        logging.info("Send to " + self.registryName + ":" + str(self.registryPort) + " -> " + message)
        response = self.tcpClientSocket.recv(1024).decode().split()
        logging.info("Received from " + self.registryName + " -> " + " ".join(response))
        
        if response[0] == "ROOM-DOESNT-EXIST":
            print("Room doesnt exist!\n")
            
        elif response[0] == "USER-IS-ALREADY-IN-ROOM":
