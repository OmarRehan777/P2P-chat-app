    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #   SEARCH  #
                    ...
                    if db.is_account_exist(message[1]):
                        ...
                        if db.is_account_online(message[1]):
                            ...
                            #peer_info[0] = ip, peer_info[1] = port
                ...
                #join room chat
                elif message[0]== "JOINROOMCHAT":
                    if not db.is_room_exist(message[1]):
                        response = "ROOM-DOESNT-EXIST"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
                        
                    elif db.isUserAlreadyInRoom(message[1], message[2]):
                        response = "USER-IS-ALREADY-IN-ROOM"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
